...
#ifdef WITH_PERFSCHEMA_STORAGE_ENGINE
  pre_initialize_performance_schema();
#endif /*WITH_PERFSCHEMA_STORAGE_ENGINE   初始化perdormance_schema，<-重启后会丢失之前的数据*/
/*通过WITH_PERFSCHEMA_STORAGE_ENGINE宏来决定是否编译，
  启用后会自动启用HAVE_PSI_INTERFACE, 
	  该宏会影响mysql_thread_create的行为
	  http://www.ilongda.com/2020/11/09/performance_schema/
	  */
